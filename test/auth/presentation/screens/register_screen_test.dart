...
void main() {
  ...
  group('RegisterScreen', () {
    ...
    testWidgets('navigates back to login screen when login link is tapped',
        (tester) async {
      await tester.pumpWidget(createRegisterScreen());
      await tester.pump();
      ...
      verify(appNavigator.pop()).called(1);
    });

    testWidgets('register form validates all fields before submission',
        (tester) async {
      ...
      await tester.enterText(
          find.byKey(const Key('registerForm_nameInput_textField')),
          'Test User');
      await tester.enterText(
    ...
    testWidgets('prevents multiple submissions while loading', (tester) async {
      ...
      verifyNever(authCubit.register(
        name: anyNamed('name'),
        email: anyNamed('email'),
        password: anyNamed('password'),
      ));
